...
class Moogento_Core_Model_Observer
{
    ...
    {
        if(!defined("_MPDF_SYSTEM_TTFONTS")) define("_MPDF_SYSTEM_TTFONTS", Mage::getBaseDir('lib').DS.'mpdf'.DS.'ttfonts'.DS);
        ...
        if (Mage::helper('moogento_core')->isInstalled('BL_CustomGrid') && Mage::getStoreConfigFlag('moogento_shipeasy/general/override_bl_grid')) {
            ...
            try {
                $value = Mage::helper('core/string')->unserialize($value);
            } catch (Exception $e) {
    ...
    public function resolveConflictForIwdOrderGrid()
    {
        if (!Mage::helper('moogento_core')->isInstalled('IWD_OrderGrid')) {
            return $this;
        }
        ...
        <config>
            <global>
                <blocks>
                    <adminhtml><rewrite>
                        <sales_order>Moogento_Core_Block_Adminhtml_Sales_Order_Rewrite_IWDOrderGrid_Grid</sales_order>
                    </rewrite></adminhtml>
                    <moogento_core><rewrite>
                        <adminhtml_sales_order_grid>Moogento_Core_Block_Adminhtml_Sales_Order_Rewrite_IWDOrderGrid_Grid</adminhtml_sales_order_grid>
                    </rewrite></moogento_core>
                </blocks>
                <models>
                    <iwd_ordergrid><rewrite>
                        <order_grid>Moogento_Core_Model_Order_Rewrite_IWDOrderGrid_Grid</order_grid>
                    </rewrite></iwd_ordergrid>
                    <sales_resource><rewrite>
                        <order_grid_collection>Moogento_Core_Model_Mysql4_Sales_Order_Grid_Rewrite_IWDOrderGrid_Collection</order_grid_collection>
                    </rewrite></sales_resource>
                </models>            
            </global>
        </config>
        ');
    ...
    {
        if(!defined("_MPDF_SYSTEM_TTFONTS")) define("_MPDF_SYSTEM_TTFONTS", 'D:/www/moogento-modules/lib/mpdf/ttfonts/');
            ...
            if (!Mage::registry('moogento_cron_checked') && time() - $lastCheck > 30 * 60) {
                ...
                if ($cronProblems) {
                    Mage::getSingleton('adminhtml/session')
                        ->addError(Mage::helper('moogento_core')->__('Warning! Your Magento cron seems to not be running. You need this for your Moogento extensions to run well.') . ' (<a target="_blank" href="https://moogento.com/guides/Installing_Magento_Extensions_:_Pre-install_Checklist#3._Check_your_cron_is_running?utm_source=install&utm_medium=config&utm_campaign=cron_warning">' . Mage::helper('moogento_core')->__('Help') . '</a>)');
                }

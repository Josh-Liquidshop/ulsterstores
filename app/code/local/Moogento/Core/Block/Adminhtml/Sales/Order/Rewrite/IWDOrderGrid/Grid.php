...
class Moogento_Core_Block_Adminhtml_Sales_Order_Rewrite_IWDOrderGrid_Grid
...
//    protected $_initialGrid = null;
    ...
    public function __construct()
    {
        ...
        if (Mage::helper('moogento_core')->isInstalled('Fooman_OrderManager')) {
            if (Mage::getStoreConfig('ordermanager/settings/displaytrackingcolumn')) {
                $this->addCustomColumn('fooman_tracking', array(
                    'header' => Mage::helper('ordermanager')->__('Tracking #'),
                    'type' => 'text',
                    'sortable' => false,
                    'index' => 'fooman_tracking',
                    'renderer' => 'Fooman_OrderManager_Block_Adminhtml_Widget_Grid_Column_Renderer_TrackingNumberInput',
                    'filter' => 'Fooman_OrderManager_Block_Adminhtml_Widget_Grid_Column_Renderer_TrackingNumberInput',
                ));
            }
            if (Mage::getStoreConfig('ordermanager/settings/displaycarriercolumn')) {
                $this->addCustomColumn('fooman_carrier', array(
                    'header' => Mage::helper('ordermanager')->__('Carrier'),
                    'type' => 'text',
                    'sortable' => false,
                    'index' => 'fooman_carrier',
                    'renderer' => 'Fooman_OrderManager_Block_Adminhtml_Widget_Grid_Column_Renderer_TrackingCarrierInput',
                    'filter' => 'Fooman_OrderManager_Block_Adminhtml_Widget_Grid_Column_Renderer_TrackingCarrierInput',
                ));
            }
        }
        if (Mage::helper('moogento_core')->isInstalled('GoSolid_ShippingExport')) {
            $this->addExportType('*/shippingexport_index/exportReadyToShipCsv', Mage::helper('sales')->__('Ready to Ship CSV'));
        }
    }
    ...
    protected function _prepareCollection()
    {
        ...
        if (!$this->_hasProfitFieldInCollection()) {
            $this->getCollection()->addFieldToSelect('profit_amount');
        }
    ...
    protected function _getInitialGrid()
    {
        if (is_null($this->_initialGrid)) {
            ...
            $this->_initialGrid->setDefaultSort(false);
    ...
    protected function _hasProfitFieldInCollection()
    {
        $columns = $this->getCollection()->getSelect()->getPart('columns');
        ...
        foreach ($columns as $column) {
            list($colTable, $colName) = $column;

            if ($colTable === 'main_table' &&
                $colName === 'profit_amount'
            ) {
                return true;
            }
        }
    ...
    protected function _beforeToHtml()
    {
        if (Mage::helper('iwd_ordergrid')->isEnabled()) {
            ...
            if (in_array('adminhtml_sales_order_grid', $handles, true)) {
                $jsInitBlock = $layout
                    ->createBlock('iwd_ordergrid/adminhtml_sales_order_grid_init', 'iwd_om.order.grid.jsinit')
                    ->setTemplate('iwd/ordergrid/grid/jsinit.phtml');

                $this->append($jsInitBlock);
            }
        }

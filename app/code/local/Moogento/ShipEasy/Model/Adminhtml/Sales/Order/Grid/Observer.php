...
class Moogento_ShipEasy_Model_Adminhtml_Sales_Order_Grid_Observer
{
        ...
        if (Mage::getStoreConfig('moogento_shipeasy/orderpage/button_scan_enable')) {
            $block->addButton(
                ...
                array(
                    ...
                    'class' => 'tooltipster moo_process_scan',
                    'title' => Mage::helper('moogento_shipeasy')->__('Scan Order ID barcodes and set their status'),
                )
            );
        }
        ...
        if (Mage::getStoreConfig('moogento_shipeasy/import/import_shipment')) {
            $block->addButton(
                ...
                array(
                    ...
                    'class' => 'tooltipster import',
                    'title' => Mage::helper('moogento_shipeasy')->__('Set order statuses by importing a CSV'),
                )
            );
        }
    ...
    public function moogento_core_order_grid_columns($observer)
    {
        ...
        $grid->addCustomColumn('paid', array(
                'header' => Mage::helper('moogento_shipeasy')->__('Revenue'),
        ...
        $grid->addCustomColumn('tax', array(
            'header' => Mage::helper('moogento_shipeasy')->__('Tax'),
            'renderer' => 'moogento_shipeasy/adminhtml_widget_grid_column_renderer_tax',
            'index' => 'amount',
            'type' => 'currency',
            'width' => '10em',
            'currency' => 'order_currency_code'
        ));
        $grid->addCustomColumn('shippaid', array(
            'header' => Mage::helper('moogento_shipeasy')->__('Shipping (paid)'),
            'renderer' => 'moogento_shipeasy/adminhtml_widget_grid_column_renderer_shippaid',
        ...
        $grid->addCustomColumn('weight', array(
                ...
                'type' => 'number',
                'width' => '10em',
                'column_css_class' => 'customer_weight_list non-edit',
                'renderer' => 'moogento_shipeasy/adminhtml_widget_grid_column_renderer_weight',
            ));
        ...
        if (Mage::helper('moogento_core')->isInstalled('Camiloo_Channelunity')) {
            $grid->addCustomColumn('channel_unity_origin', array(
                'header' => Mage::helper('moogento_shipeasy')->__('channelUnity'),
                'index' => 'channel_unity_origin',
                'filter_index' => 'main_table.channel_unity_origin',
                'width' => '40px',
                'default' => '<p style="color:grey;font-style:italic;" title="">' . Mage::helper('moogento_shipeasy')->__("Pending Sync") . '</p>',
                'renderer' => 'moogento_shipeasy/adminhtml_widget_grid_column_renderer_channel_unity_origin',
            ));
        }
        ...
        if (Mage::helper('moogento_core')->isInstalled('Ess_M2ePro') || Mage::helper('moogento_core')->isInstalled('Camiloo_Channelunity')) {
            $grid->addCustomColumn('mkt_order_id', array(
                ...
                'renderer' => 'moogento_shipeasy/adminhtml_widget_grid_column_renderer_cron',
                'default' => '<p style="color:grey;font-style:italic;" title="">' . Mage::helper('moogento_shipeasy')->__("Pending Sync") . '</p>',
                'filter_condition_callback' => array($this, '_customMktorderidFilterCallBack'),
            ));
        }
    }

    public function _customMktorderidFilterCallBack($collection, $column)
    {
        if (!$value = $column->getFilter()->getValue())
        {
            return $this;
        }
        $collection->getSelect()->where("main_table.`mkt_order_id` like ?", "%$value%");
        ...
        switch ($columnId) {
            ...
            case 'szy_status':
                ...
                if ($existingStatusGroups) {
                    try {
                        $existingStatusGroups = Mage::helper('core/string')->unserialize($existingStatusGroups);

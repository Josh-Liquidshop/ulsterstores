...
class Moogento_ShipEasy_Model_Adminhtml_Sales_Order_Grid_Collection_Observer
{
    ...
    public function moogento_core_order_grid_collection_prepare($observer)
    {
        ...
        if (Mage::getStoreConfig('moogento_shipeasy/grid/common_filter_status')) {
            if(empty($grid)){
                $grid = $observer->getEvent()->getGrid();
            }
            $grid->setDefaultFilter(array('szy_status' => Mage::getStoreConfig('moogento_shipeasy/grid/common_filter_status')));
        }

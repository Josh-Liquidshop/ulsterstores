...
HEREDOC;
            ...
            foreach ($itemsCollection as $item) {
                ...
                $order_weight += $item->getWeight() * $item->getQtyOrdered();
            }
            $order->setWeight($order_weight);
        }
    ...
    public function sales_order_shipment_save_before($observer)
    {
		...
		if(null !== $shipment->setAdminUserId(Mage::getSingleton('admin/session')->getUser())){
	        $shipment->setAdminUserId(Mage::getSingleton('admin/session')->getUser()->getId());

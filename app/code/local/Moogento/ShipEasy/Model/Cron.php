        ...
        if (Mage::helper('moogento_core')->isInstalled('Camiloo_Channelunity')) {
            ...
            foreach ($orders as $order) {
                ...
                if (count($transactions)) {
                    ...
                    $infoArray = $transaction->getAdditionalInformation();
                    if (isset($infoArray['ServiceType'])) {
                        $order->setData('channel_unity_origin', $infoArray['ServiceType']);
                    } else {
                        $order->setData('channel_unity_origin', '');
                    }
                    try {
                        $order->save();
                    } catch (Exception $e) {
        ...
        if (Mage::helper('moogento_core')->isInstalled('Ess_M2ePro')
            ...
            foreach ($orders as $order) {
                ...
                if (($additional_data != '') && (!is_null($additional_data))) {
                    $data = Mage::helper('core/string')->unserialize($additional_data);

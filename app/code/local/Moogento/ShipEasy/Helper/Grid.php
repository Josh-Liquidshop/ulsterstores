...
class Moogento_ShipEasy_Helper_Grid extends Mage_Core_Helper_Abstract
{
    ...
    public function getMassActionItems()
    {
        ...
        if ($this->_isAllowed('moo_shipeasy_assign_tracking')) {
            if (Mage::getStoreConfig('moogento_shipeasy/action_menu/show_assign_tracking') == 1) {
        ...
        if ($this->_isAllowed('moo_shipeasy_change_update_custom_attribute')) {
            if (Mage::getStoreConfig('moogento_shipeasy/action_menu/show_update_custom_attribute') == 1) {
                $items['custom_order_attribute'] = array(
                    'label'      => 'sE ' . Mage::helper('moogento_shipeasy')->__('Update shipEasy flag '),
    ...
    public static function setPaidFilter($collection, $column)
    {
        ...
        if (isset($filterValue['from'])) {
            $collection->getSelect()->where('ifnull(main_table.total_paid, main_table.grand_total) >= ?', $filterValue['from']);
        }
        if (isset($filterValue['to'])) {
            $collection->getSelect()->where('ifnull(main_table.total_paid, main_table.grand_total) <= ?', $filterValue['to']);
        }
    }

                ...
                switch ($component_mode){
                    case "amazon":
                        ...
                        foreach($magento_order->getAllItems() as $index => $item){
                            $additionalData = Mage::helper('core/string')->unserialize($item->getAdditionalData());
                            if ($additionalData && isset($additionalData['m2epro_extension']['items'][0]['order_item_id'])) {
                                ...
                                if ($product->getId()) {
									...
									// fix m2Epro returning incorrect store link - assuming that remote customer ordered from the same store they're registered to!
									if( isset($data['data']['data']) && (strpos($data['data']['data'], 'amazon.co.uk') !== false) )
										 $data['products'][$index]['merchant_link'] = str_ireplace('amazon.com','amazon.co.uk',  $data['products'][$index]['merchant_link']);
                                }
                            }
                        }

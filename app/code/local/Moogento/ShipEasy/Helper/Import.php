...
class Moogento_ShipEasy_Helper_Import extends Mage_Core_Helper_Abstract
{
    ...
    public function saveRow(array $importData, $additionalParams = array())
    {
        ...
        if ($order->getData('state') == Mage_Sales_Model_Order::STATE_HOLDED) {
            Mage::log('Order #' . $order->getIncrementId() . ' is on hold, so we can\'t process it.','moogento_shipeasy_import.log');
        }
    ...
    protected function _invoiceOrder($order, $importData = array(), $additionalParams = array())
    {
        ...
        try {
            ...
            Mage::log($e->getMessage() . 'when processing order #' . $order->getIncrementId(),'moogento_shipeasy_import.log');
        }
    }
    ...
    protected function _updateOrder($order, $importData, $additionalParams)
    {
        ...
        if (!$message) {
            if (
                ...
                $message = 'Order ' . $order->getIncrementId() . ' : ' . Mage::helper('moogento_shipeasy')->__(
                        ' Skipping this row for import, no action taken'
                    );
            }
        }
            ...
            if ($key == 'add_track_email') {
                ...
                switch ($key) {
                    ...
                    case 'sku_column':
                        ...
                        break;
                    case 'sku_prefix_text':
                        $note[]
                            = ' (optional. With sku also specified, it will add this prefix to the imported sku, before attempting to match it in Magento)';
                        break;	
                    case 'qty_column':
                        $note[]
...
System > Config > ShipEasy >"Process Orders : CSV"';

...
// 			$productId = $postData['product_id']);
    ...
    public function batchRunAction()
    {
        if ($this->getRequest()->isPost()) {
            ...
            $batchParams = Mage::helper('core/string')->unserialize($batchModel->getData('params'));
    ...
    private function _savedImportInvoice()
    {
        $session = Mage::getSingleton('core/session');
        $notifyCustomer = $session->getData('notify_customer_for_special_import');
        $session->unsetData('notify_customer_for_special_import');

        if($session->getData('import_data_for_save_invoice')){
            $orders_data = $session->getData('import_data_for_save_invoice');
            $session->unsetData('import_data_for_save_invoice');
                ...
                foreach($order_data as $tracking_number => $data) {
                    ...
                    foreach($order->getAllItems() as $item) {
                        if(array_key_exists($item->getSku(),$data)){
                            ...
                            if ($itemsarray[$item_id] > (int)$item->getQtyOrdered() - (int)$item->getQtyCancelled() - (int)$item->getQtyInvoiced()) {
                                $itemsarray[$item_id] = (int)$item->getQtyOrdered() - (int)$item->getQtyCancelled() - (int)$item->getQtyInvoiced();
                            }
                    ...
                    if($order->canInvoice() && count($itemsarray)){
                        $invoiceId = Mage::getModel('sales/order_invoice_api')->create($order->getIncrementId(), $itemsarray ,'Created by import' ,false,1);
                        if ($tracking_number != '__notracking__') {
                            Mage::getModel('sales/order_shipment_api')->addTrack(
                                $invoiceId, 'custom', 'Custom', $tracking_number
                            );
                        }
                        if($notifyCustomer){
                            try{
                                $invoice = Mage::getModel('sales/order_invoice')->load($invoiceId, 'increment_id');
                                $invoice->sendEmail($notifyCustomer);
                                $invoice->setEmailSent($notifyCustomer);
                                $invoice->save();
                                Mage::log('Send email for invoice #'.$invoiceId.' : '.date('d/m/y H:i.s'), null, 'szy.log');
                            } catch (Exception $ex) {

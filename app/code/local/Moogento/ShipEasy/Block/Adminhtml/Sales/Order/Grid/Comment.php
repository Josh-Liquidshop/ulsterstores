...
class Moogento_ShipEasy_Block_Adminhtml_Sales_Order_Grid_Comment extends Mage_Adminhtml_Block_Template
{
    ...
    protected function _getComments()
    {
        ...
        $order = $this->getOrder();
        $statusHistoryCollection = Mage::getResourceModel('sales/order_status_history_collection')
            ->setOrderFilter($order)
            ->setOrder('created_at', 'desc')
            ->setOrder('entity_id', 'desc')
            ->setPageSize((int)$maxCount);

        foreach ($statusHistoryCollection as $comment) {
	    ...
	    if (Mage::helper('moogento_core')->isInstalled('TM_FireCheckout') && $displayed < $maxCount) {
			$orderOneStep = Mage::getModel('sales/order')->load($order->getId(),'entity_id');
        ...
        if (Mage::helper('moogento_core')->isInstalled('MW_Onestepcheckout') && $displayed < $maxCount) {
            $orderOneStep = Mage::getModel('onestepcheckout/onestepcheckout')->load($order->getId(), 'sales_order_id');
            if ($orderOneStep->getId()) {
                $displayed++;
                $comments[] = array(

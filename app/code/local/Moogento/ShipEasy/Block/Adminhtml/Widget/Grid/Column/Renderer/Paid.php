    ...
    extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Currency
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        if (Mage::getStoreConfigFlag('moogento_shipeasy/grid/paid_take_refunds')) {
            $credit_memos = $row->getCreditmemosCollection();
            foreach ($credit_memos as $credit_memo) {
                $paid -= (float)$credit_memo->getData('grand_total');
                $basePaid -= (float)$credit_memo->getData('base_grand_total');
            }
        }

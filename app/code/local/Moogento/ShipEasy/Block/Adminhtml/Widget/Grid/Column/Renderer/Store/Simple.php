    ...
    public function render(Varien_Object $row)
    {
        ...
        if ($orderStoreId) {
            ...
            if ($websiteModel) {
                ...
                if ($show_type == 1) {
                    $list = Mage::helper('core/string')->unserialize(Mage::getStoreConfig('moogento_shipeasy/grid/szy_website_id_images'));
                    if ($list && isset($list[$websiteModel->getCode()]) && $list[$websiteModel->getCode()]) {
                        $label = '<img src="' . Mage::getBaseUrl('media') . DS . 'moogento/shipeasy/szy_websites/' . $list[$websiteModel->getCode()] . '" style="height: 25px !important;" />';
                    }
                }
            }
        }

...
//                 return Mage::helper('adminhtml')->__('All Store Views');
        ...
        if (Mage::getStoreConfigFlag('moogento_shipeasy/grid/szy_store_id_format_store_view') == 0) {
            ...
            if ($orderStoreId) {
                ...
                if ($store_view) {
                    $list = Mage::helper('core/string')->unserialize(Mage::getStoreConfig('moogento_shipeasy/grid/szy_store_id_images'));
                    if ($list && isset($list[$store_view->getCode()]) && $list[$store_view->getCode()]) {
                        $label
                            = '<img style="height: 25px !important;" title="' . $store_view->getName() . '" src="'
                              . Mage::getBaseUrl('media') . DS . 'moogento/shipeasy/szy_stores/' . $list[$store_view->getCode()]

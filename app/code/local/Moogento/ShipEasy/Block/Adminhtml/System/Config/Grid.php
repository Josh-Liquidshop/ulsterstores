    ...
    Varien_Data_Form_Element_Renderer_Interface
{
    ...
    protected function _getAdditionalFields($columnId)
    {
        ...
        switch ($columnId) {
            ...
            case 'paid':
                ...
                $fields[] = array(
                    'key' => 'take_refunds',
                    'label' => $this->__('Take refunds into account'),
                    'type' => 'checkbox',
                    'checked' => Mage::getStoreConfig('moogento_shipeasy/grid/' . $columnId . '_take_refunds'),
                );
                break;
            case 'admin_comments':
                $fields[] = array(
            ...
            case 'szy_product_names':
                ...
                break;
            case 'szy_status':
                $value = Mage::helper('core/string')->unserialize(Mage::getStoreConfig('moogento_shipeasy/grid/' . $columnId . '_status_group'));
                $values = array();
                if (is_array($value)) {
            ...
            case 'szy_store_id':
                ...
                $value = Mage::helper('core/string')->unserialize(Mage::getStoreConfig('moogento_shipeasy/grid/' . $columnId . '_images'));
                $values = array();
                if (is_array($value)) {
                    foreach ($value as $name => $image) {
                        $values[] = array(
                            'name'     => $name,
                            'image' => Mage::getBaseUrl('media').DS.'moogento/shipeasy/szy_stores/' . $image,
                            'old_image' => $image,
                        );
                    }
                }
            ...
            case 'szy_store_name':
                ...
                if (is_array($value)) {
                    foreach ($value as $name => $image) {
                        $values[] = array(
                            'name'     => $name,
                            'image' => Mage::getBaseUrl('media').DS.'moogento/shipeasy/szy_groups/' . $image,
                            'old_image' => $image,
                        );
                    }
                }
                ...
                foreach (Mage::app()->getWebsites() as $website) {
                    foreach ($website->getGroups() as $group) {
                        $storeList[] = array(
                            'label' => $website->getName() . ' -> ' . $group->getName(),
                            'value' => $group->getId(),
                        );
                    }
            ...
            case 'szy_website_id':
                ...
                if (is_array($value)) {
                    foreach ($value as $name => $image) {
                        $values[] = array(
                            'name'     => $name,
                            'image' => Mage::getBaseUrl('media').DS.'moogento/shipeasy/szy_websites/' . $image,
                            'old_image' => $image,
                        );
                    }
                }
    ...
    You can use your own by saving .png images in the /skin/adminhtml/default/default/moogento/images/flag_images/ folder and using the filename here.';
            ...
            case 'szy_shipping_method':
                $values = Mage::helper('core/string')->unserialize(Mage::getStoreConfig('moogento_shipeasy/grid/szy_shipping_method_method_group'));
                if (!$values) {
                    $values = array();
                }
            ...
            case 'timezone':
                ...
                break;
            case 'channel_unity_origin':
                $fields[] = array(
                    'key' => 'replace_with_image',
                    'label' => $this->__('Replace with image?'),
                    'type' => 'checkbox',
                    'checked' => Mage::getStoreConfig('moogento_shipeasy/grid/' . $columnId . '_replace_with_image'),
                );
                $value = Mage::helper('core/string')->unserialize(Mage::getStoreConfig('moogento_shipeasy/grid/' . $columnId . '_images'));
                $values = array();
                if (is_array($value)) {
                    foreach ($value as $name => $image) {
                        $values[] = array(
                            'name'     => $name,
                            'image' => Mage::getBaseUrl('media').DS.'moogento/shipeasy/channelunity/' . $image,
                            'old_image' => $image,
                        );
                    }
                }
                $fields[] = array(
                    ...
                    'label' => $this->__("Images"),
                    'type' => 'serializable_table',
                    'visible' => array('replace_with_image' => 1),
                    'fields' => array(
                        array(
                            'key' => 'name',
                            'label' => $this->__("Name"),
    ...
    {
        $result = '<p>';
        $result .= $this->__('(First pageload) Pre-filter by status:') . ': ';
        $result .= '<select class="chosen" name="groups[grid][fields][common_filter_status][value]">';
        $result .= '<option value=""></option>';
        $statuses = Mage::getModel('sales/order_status')->getCollection();
        foreach ($statuses as $status) {
            if (Mage::getStoreConfig('moogento_shipeasy/grid/common_filter_status') == $status->getStatus()) {
                $result .= '<option  value="' . $status->getStatus() . '" selected="selected">' . $status->getLabel() . '</option>';
            } else {
                $result .= '<option value="' . $status->getStatus() . '">' . $status->getLabel() . '</option>';
            }
        }
        $result .= '</select>';
        ...
        $result .= '<p id="presort">';
            ...
            $result .= '<select id="presort_select" class="chosen" name="groups[grid][fields][sorting_group_type_field][value]" onchange="changingPresortingGroup();">';
                ...
                if(Mage::helper("moogento_core")->isInstalled("Moogento_CourierRules")){
                    if(Mage::getStoreConfig('moogento_shipeasy/grid/sorting_group_type_field') == "courierrules"){
                        $result .= '<option value="courierrules" selected="selected">'.$this->__("shippingRules Method").'</option>';
                    } else {
                        $result .= '<option value="courierrules">'.$this->__("shippingRules Method").'</option>';
                    }
                }
            $result .= '</select>';
            ...
            if(Mage::helper("moogento_core")->isInstalled("Moogento_CourierRules")){
                $values = Mage::helper('core/string')->unserialize(Mage::getStoreConfig('moogento_shipeasy/grid/courierrules_description_status_group'));
                if (!$values) { $values = array(); }
                $value = Mage::getStoreConfig('moogento_shipeasy/grid/sorting_group_courier_rule_field');

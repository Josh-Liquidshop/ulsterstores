...
class Amasty_Pgrid_Block_Adminhtml_Catalog_Product_Grid extends Mage_Adminhtml_Block_Catalog_Product_Grid
{
    ...
    public function setCollection($collection)
    {
        ...
        if ($store->getId() == 0) {
            ...
            $flatTableId = $firstStore->getId();
        } else {
            $flatTableId = $store->getId();
        }
        ...
        if (0 < $this->_gridAttributes->getSize()) {
            $joinIndexTable = false;
            $flatTableName = sprintf(
                '%s_%s',
                Mage::getSingleton('core/resource')->getTableName('catalog/product_flat'),
                $flatTableId
            );
            foreach ($this->_gridAttributes as $attribute) {
                if ($attribute->getUsedInProductListing() && Mage::helper('catalog/product_flat')->isAvailable()) {
                    if (!$joinIndexTable) {
                        $collection->joinTable(
                            $flatTableName,
                            'entity_id=entity_id',
                            array($attribute->getAttributeCode() => $attribute->getAttributeCode()),
                            null,
                            'left'
                        );
                        $joinIndexTable = true;

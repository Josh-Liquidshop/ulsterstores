...
class Amasty_Pgrid_Adminhtml_Ampgrid_FieldController extends Mage_Adminhtml_Controller_Action
{
    ...
    */
    ...
    {
        if (Mage::helper('core')->isModuleEnabled('Amasty_Rolepermissions')) {
    ...
    protected function _updateProductData($productId, $field, $value)
    {
        ...
        if ($this->_product) {
            Mage::register('amasty_grid_product_data', $this->_product->getData(), true);
            $result  = array();
            ...
            $productData = $this->_product->getData();

            foreach ($productAttributes as $value) {
                if (!isset($productData[$value])) {
                    $this->_product->setData($value, false);
                } else {
                    $this->_product->setData($value, $productData[$value]);
                }
            }
    ...
    /**
     * Get all eav product attributes
     *
     * @return array
     */
